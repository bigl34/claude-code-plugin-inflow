...
- An open inFlow browser tab (logged in) — needed to extract the ephemeral Bearer token
...
Ask the user for:
1. **Order number** — PO number (e.g. `PO-000011`) or SO number
2. **File paths** — absolute paths to files to upload

If the user has already provided these, proceed directly.

## Step 2 — Extract Bearer Token

The inFlow Cloud API uses session-based Bearer tokens that are NOT stored in localStorage/sessionStorage (HTTP-only cookies). Extract by intercepting XHR in the live inFlow browser tab.

### 2a. Find the inFlow tab
...
**Also fetch the current PO with includes** — you'll need this for the linking step:
...
- `included[]` entries for existing attachments

## Step 4 — Upload Files

For each file, perform two API calls:

### 4a. Get signed upload URL
...
### 4b. PUT file to Azure Blob Storage
...
### 4c. Collect attachment metadata

For each uploaded file, build an attachment record:

```json
{
  "attachmentId": "<data.id from 4a>",
  "fileName": "<original filename>",
  "fileSize": <file size in bytes from disk>,
  "attachmentUrl": "<blobRelativeUrl from 4a>",
...
## Step 5 — Link Attachments to Order
...
Report to user: "Uploaded N file(s) to PO-XXXXXX. Total attachments: M."
...
8. **Bearer token expires.** Tokens are session-based and expire after hours. If you get 401, re-extract from the browser tab.
